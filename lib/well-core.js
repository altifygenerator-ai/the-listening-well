...
1. Read any ADDITIONAL DETAIL as part of the same wish. It exists because the original wish needed one more concrete fact.
2. Identify the literal subject: who or what the wish is about.
3. Identify the concrete outcome the person wants.
4. Identify the strongest tension, obstacle, uncertainty, or emotional stake explicitly supported by their words.
5. Separate what the person can influence from what depends on another person, chance, timing, institutions, health, markets, or other outside forces.
6. Notice any unusually specific detail they supplied: a person, event, time frame, number, place, decision, repeated behavior, loss, fear, or tradeoff.
7. Decide what the response should directly answer. Do not reveal this analysis.
...
- Match the emotional weight of the wish. A playful, ordinary, or whimsical wish should stay light; do not manufacture hidden trauma or a life lesson. A serious wish deserves seriousness without melodrama.
- If the wish depends on another person's feelings or choices, never claim to know what that person thinks, wants, or will do.
- If the wish is impossible, fantastical, or clearly joking, preserve the fun and respond to the spirit of it rather than pretending it is a prediction.
...
- MOON WATER should feel like a different PRODUCT, not a longer paragraph. Give the direct answer first, then a grounded perspective shift the free response did not provide. The moonNote should feel like a private note worth saving and revisiting, not prophecy. Do not pad the response simply because it is premium.
...
- moonNote: null for STANDARD, FREE CLARIFICATION, and DEEP WATER. For MOON WATER only, write 2–3 concise sentences titled in spirit as a note from farther down the path. It must stay grounded in the supplied facts, name a useful perspective shift, and never predict an outcome.
...
export function localWishResponse(wish, { depth = "standard", deep = false } = {}) {
  ...
  return {
    ...
    moonNote: mode === "moon" ? "If this situation changes, the useful thing to notice may be which part changed because of your choices and which part changed on its own. That distinction can help you carry the wish with more clarity next time." : null,
...
export function parseModelJson(text) {
  ...
  return {
    ...
    moonNote: typeof value.moonNote === "string" && value.moonNote.trim() ? value.moonNote.trim().slice(0, 650) : null,
...
const WELL_RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    moonNote: { anyOf: [{ type: "string" }, { type: "null" }] },
  ...
  required: ["answer", "meaning", "nextStep", "shareLine", "followUpQuestion", "moonNote", "mood", "theme"],
...
const WELL_QUALITY_SCHEMA = {
  ...
  required: ["specificity", "directness", "grounding", "usefulness", "genericRisk", "inventedDetails", "critique"],
  additionalProperties: false
};
...
async function evaluateWishResponse({ wish, clarification = "", followUp, response, apiKey, model, safetyIdentifier = "" }) {
  const context = followUp?.question
    ? `ORIGINAL WISH:
${wish}${followUp.originalClarification ? `

ADDITIONAL DETAIL:
${String(followUp.originalClarification).slice(0, 500)}` : ""}

PRIOR ANSWER:
${String(followUp.originalAnswer || "").slice(0, 700)}
...
ADDITIONAL DETAIL:
${String(clarification).slice(0, 500)}` : ""}`;
  const payload = {
    model,
    store: false,
    max_output_tokens: 280,
    instructions: `You are a ruthless human product editor for a private reflection product. Judge whether the proposed response would make a real person feel understood and whether it gives them anything useful beyond generic AI wisdom. Score specificity, directness, grounding, and usefulness separately. A polished generic answer is a failure. Do not reward poetry. Do not require private details to be repeated verbatim. A score of 4 means clearly good enough to ship; 5 means unusually precise. Set genericRisk true if the answer could fit many unrelated wishes. Set inventedDetails true if it asserts motives, history, relationship dynamics, diagnoses, facts, or obstacles the user did not provide. Penalize stock advice such as simply telling the person to focus, let go, set boundaries, take a small step, or control what they can unless that advice is tied to a specific fact from this wish. Keep critique under 70 words and name the single most important correction.`,
    input: `${context}
...
const WISH_READINESS_SCHEMA = {
  type: "object",
  properties: {
    ready: { type: "boolean" },
    question: { type: "string" },
    reason: { type: "string", enum: ["specific_enough", "missing_subject", "missing_outcome", "missing_context"] }
  },
  required: ["ready", "question", "reason"],
  additionalProperties: false
};

export async function assessWishReadiness({ wish, apiKey, model = "gpt-5", safetyIdentifier = "" }) {
  const cleanWish = sanitizeWish(wish).slice(0, 700);
  if (!apiKey || !cleanWish) return { ready: true, question: "", reason: "specific_enough" };
  const payload = {
    model,
    store: false,
    max_output_tokens: 180,
    instructions: `Decide whether a private wish contains enough concrete information for a response to feel genuinely specific without inventing facts. Be permissive: a short wish is fine if it clearly names what the person wants and gives at least one useful situational detail. Ask for clarification only when the answer would otherwise have to guess. If clarification is needed, ask exactly one natural question under 18 words. Do not ask for sensitive details, names, diagnoses, exact finances, or anything unnecessary. The question should target the single missing fact that would most improve the response.`,
    input: `PRIVATE WISH:
${cleanWish}`,
    text: {
      format: {
        type: "json_schema",
        name: "wish_readiness",
        description: "Whether a private wish needs one more detail before a useful response can be generated.",
        strict: true,
        schema: WISH_READINESS_SCHEMA
      }
    }
  };
  if (safetyIdentifier) payload.safety_identifier = crypto.createHash("sha256").update(String(safetyIdentifier)).digest("hex").slice(0, 48);
  ...
  return {
    ...
    question: Boolean(parsed.ready) ? "" : String(parsed.question || "What would actually be different if this wish came true?").trim().slice(0, 180),
    reason: ["specific_enough", "missing_subject", "missing_outcome", "missing_context"].includes(parsed.reason) ? parsed.reason : "missing_context"
  };
}
...
FOLLOW-UP QUESTION:
${String(followUp.question || "").slice(0, 320)}${followUp.originalClarification ? `

ADDITIONAL DETAIL FROM THE ORIGINAL WISH:
${String(followUp.originalClarification).slice(0, 500)}` : ""}`
...
Rewrite from scratch. Be more concrete and direct rather than merely adding words.` : ""}`,
  ...
  try {
    ...
    if (!qualityPassed(review)) {
      ...
      const retry = parseModelJson(responseOutputText(retryData));
      try {
        const retryReview = await evaluateWishResponse({
          wish,
          clarification,
          followUp,
          response: retry,
        ...
        if (!qualityPassed(retryReview)) {
          const qualityError = new Error("The response did not clear the well's specificity gate after regeneration.");
          qualityError.code = "WELL_RESPONSE_QUALITY";
          throw qualityError;
        }
        chosen = retry;
      } catch (error) {
        if (error?.code === "WELL_RESPONSE_QUALITY") throw error;
        // If the editor itself is unavailable after a rewrite, prefer the rewritten draft
        // rather than making a transient reviewer failure break the whole experience.
        chosen = retry;
      }
    }
  } catch (error) {
    if (error?.code === "WELL_RESPONSE_QUALITY") throw error;
    // A reviewer outage should not make the well unavailable when generation succeeded.
